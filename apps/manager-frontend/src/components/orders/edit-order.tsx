...
export default function EditOrder({ isOpen, onClose, order, onOrderUpdate }: EditOrderProps) {
    ...
    const handleItemChange = (index: number, field: string, value: string) => {
        ...
        if (field === 'name') {
            ...
            const totalQuantity = updatedItems.reduce((sum, item, idx) => 
                idx === index ? sum : sum + item.quantity, 0
            )
    ...
    const handleAddProduct = (productName: string) => {
        ...
        // 檢查商品是否已存在
        const existingItem = editFormData.items.find(item => item.name === productName)
        if (existingItem) return
        
        // 新增商品到訂單
        const updatedItems = [...editFormData.items, { name: productName, quantity: 1 }]
                    ...
                    <div className="mb-4">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <div className="flex space-x-2">
                                ...
                                {!editFormData.items?.some(item => item.name === '黑胡椒雞胸肉') && (
                                    <button
                                        type="button"
                                ...
                                {!editFormData.items?.some(item => item.name === '照燒雞胸肉') && (
                                    <button
                                        type="button"
